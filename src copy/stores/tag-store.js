...
export const useStore = defineStore('tag-store', {
  state: () => ({
    items: [],
    items_category: [],
    items_date: [],
  ...
  actions: {
    ...
    async onRetrieveTag(payload = null) {
      const vm = this
      ...
      const items = await axios.get(host+`posts?page=${vm?.current_page}&order=${vm?.order}&per_page=${vm?.per_page}&tags=${vm.tag}`,{
          params: {
            keyword: vm?.keyword
          }
        })
