...
export const useStore = defineStore('menu-categories-store', {
  state: () => ({
    items: [],
  ...
  actions: {
    ...
    async onRetrieveMenuCategories(payload = null) {
      const vm = this
      ...
      const items = await axios.get(`categories?page=${vm?.current_page}&order=${vm?.order}&per_page=${vm?.per_page}`,{
          params: {
            keyword: vm?.keyword
          }
        })
