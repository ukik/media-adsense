...
export const useStore = defineStore('author-store', {
  state: () => ({
    items_author: [],
  ...
  actions: {
    ...
    async onRetrieveAuthor(payload = null) {
      const vm = this
      ...
      if(payload) {
        ...
        vm.author = payload?.author
      }

      const items = await axios.get(host+`posts?page=${vm?.current_page}&order=${vm?.order}&per_page=${vm?.per_page}&author=${vm.author}`,{
          params: {
            keyword: vm?.keyword
          }
        })
